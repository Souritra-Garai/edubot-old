...
) : DeadReckoning(0, 0, 0, 0, 0, 0)   
{
    ...
    init_pubs_(nh, prefix);
    init_subs_(nh, prefix);
    init_srv_servers_(nh, prefix);
}
...
void ROSDeadReckoning::init_subs_(
    ...
    vel_sub_ = node_handle.subscribe(prefix + "vel", 1000, &ROSDeadReckoning::velCb_, this);
}
...
void ROSDeadReckoning::init_pubs_(
    ...
    odom_pub_ = node_handle.advertise<nav_msgs::Odometry> (prefix + "posture",  1000);
}
...
void ROSDeadReckoning::velCb_(const geometry_msgs::TwistStamped &msg)
{
    ...
    float curr_time = msg.header.stamp.toSec();
